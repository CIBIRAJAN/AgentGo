...
class DueService {
  ...
  /// Get overdue/lapsed clients using aggregation logic.
  Future<List<AggregatedDueModel>> getAggregatedDues({
    int minMonths = 2,
    int maxMonths = 1000,
  }) async {
    final response = await _client.rpc('get_aggregated_dues', params: {
      'p_user_ids': _targetUserIds,
      'p_min_months': minMonths,
      'p_max_months': maxMonths,
    });
    if (response == null) return [];
    return (response as List).map((e) => AggregatedDueModel.fromJson(e)).toList();
  }
